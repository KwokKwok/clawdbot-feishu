...
async function convertMarkdown(client: Lark.Client, markdown: string) {
  ...
  if (res.code !== 0) {
    ...
    throw new Error(
      `Markdown conversion failed: ${res.msg} (code: ${res.code})`,
    );
  }
...
export function registerFeishuDocTools(api: OpenClawPluginApi) {
  ...
  if (!feishuCfg?.appId || !feishuCfg?.appSecret) {
    api.logger.debug?.(
      'feishu_doc: Feishu credentials not configured, skipping doc tools',
    );
    return;
  }
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            switch (p.action) {
              ...
              case 'update_block':
                return json(
                  await updateBlock(client, p.doc_token, p.block_id, p.content),
                );
              case 'delete_block':
